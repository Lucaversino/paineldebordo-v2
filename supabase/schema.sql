...
-- V202 — AIS regional: histórico de buscas 30 MN e snapshots administrativos 80 MN
...
create index if not exists idx_ais_area_searches_owner_time on public.ais_area_searches(owner_id, id desc);
...
create index if not exists idx_ais_admin_regional_visible on public.ais_admin_regional_areas(visible, id desc);
create index if not exists idx_ais_admin_regional_auto on public.ais_admin_regional_areas(auto_update, id);
