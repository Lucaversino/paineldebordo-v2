...
import { rankGfwEntries } from "../../../lib/gfw";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`https://gateway.api.globalfishingwatch.org/v3/vessels/search?${params}`, {
    ...
    if (!response.ok) {
      return reply({
        error: response.status === 429
          ? "Limite do Global Fishing Watch atingido. Aguarde antes de tentar novamente."
          : response.status === 401 || response.status === 403
            ? "Token Global Fishing Watch inválido, expirado ou sem permissão. Verifique a configuração no servidor."
            : "Global Fishing Watch indisponível no momento. Tente novamente ou use Outras fontes.",
    ...
    const vessels = rankGfwEntries(data.entries, q).slice(0, 30);
    const suggestion = data?.metadata?.didYouMean?.shipname?.[0]?.value || null;
