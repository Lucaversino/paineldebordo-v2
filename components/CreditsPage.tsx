...
export default function CreditsPage() {
  ...
  return <section className="credits-page">
    ...
    {data.wallet.balance <= 0 && <div className="credits-zero-warning" role="alert"><b>⚠ SEM CRÉDITOS</b><span>Escolha um pacote de 2, 6, 8 ou 10 créditos para continuar usando as consultas AIS.</span></div>}
    ...
    <><div className="credits-section-heading"><div><h3 className="credits-section-title">Comprar créditos</h3><p>Escolha 2, 6, 8 ou 10 créditos. Pagamento somente por PIX; o valor segue o preço por crédito configurado no painel.</p></div><span><QrCode /> PIX</span></div><div className="credits-packages">{data.packages.map((pack) => <button key={pack.credits} type="button" onClick={() => openPix(pack)}><QrCode /><b>{pack.credits} CRÉDITOS</b><span>{brl(pack.amountBrl)}</span><small>PAGAR COM PIX</small></button>)}</div></>
